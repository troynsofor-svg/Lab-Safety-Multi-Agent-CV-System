...
Perception Layer  
YOLOv11 model detects PPE items and hazards.

Classification Layer

ppe_classifier.py → Determines PPE compliance

hazard_classifier.py → Identifies unsafe conditions

Reasoning Layer  
LLM Safety Agent combines detections + classifiers → produces final safety decision.

Orchestrator Layer  
Coordinates detector, classifiers, and LLM reasoning.
...
YOLOv11 performs best with GPU acceleration.

Small dataset limitations  
Some PPE classes may have lower recall due to limited samples.

Occlusions reduce accuracy  
Goggles and gloves may be missed when partially hidden.

Hazard classes depend on dataset quality  
If hazards are under‑represented, detection may be weaker.

LLM reasoning is deterministic  
It only uses provided detections — no external knowledge.
...
matplotlib==3.8.2
tqdm==4.66.1
These versions are stable and compatible with YOLOv11.
